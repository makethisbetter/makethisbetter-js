...
describe('collectPageContext', () => {
  ...
  it('removes query and fragment from the page URL', () => {
    history.replaceState({}, '', '/reset-password?token=secret#confirm')

    expect(collectPageContext().page_url).toBe(`${location.origin}/reset-password`)
  })
})
