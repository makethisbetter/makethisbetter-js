...
describe('RecordControlBar', () => {
  ...
  it('renders the stop message as text', () => {
    const messages = createMessages()
    const payload = '<img src=x onerror="window.widgetXss=true">'
    messages.record.stop = payload
    bar = new RecordControlBar(shadow, messages, () => 0, () => {})

    const container = shadow.appended[0] as HTMLElement
    expect(container.querySelector('img')).toBeNull()
    expect(container.querySelector('.mtb-record-stop')?.textContent).toBe(payload)
  })
