...
const mockRecord = Object.assign(
  ...
  { addCustomEvent: mockAddCustomEvent, mirror: { getNode: mockGetNode } },
)
...
describe('RecordSession', () => {
  ...
  it('keeps ordinary inputs while filtering sensitive fields', async () => {
    session = new RecordSession(shadow)
    await session.start()

    const options = mockRecord.mock.calls[0][0] as MockRecordOptions
    const notes = document.createElement('input')
    notes.name = 'notes'
    const password = document.createElement('input')
    password.type = 'password'

    expect(options.maskInputFn('export is slow', notes)).toBe('export is slow')
    expect(options.maskInputFn('hunter2', password)).toBe('[Filtered]')
    expect(options.maskAllInputs).toBe(false)
    expect(options.maskInputOptions.hidden).toBe(true)
    expect(options.maskInputOptions.file).toBe(true)
    expect(options.blockClass).toBe('rr-block')
    expect(options.maskTextClass).toBe('rr-mask')
  })

  it('filters ordinary input values inside an rr-mask subtree', async () => {
    session = new RecordSession(shadow)
    await session.start()

    const options = mockRecord.mock.calls[0][0] as MockRecordOptions
    const maskedRegion = document.createElement('div')
    maskedRegion.className = 'rr-mask'
    const accountValue = document.createElement('input')
    accountValue.name = 'account_value'
    maskedRegion.appendChild(accountValue)

    expect(options.maskInputFn('Masked account value', accountValue)).toBe('[Filtered]')
  })

  it('filters credentials and URL secrets before retaining an event', async () => {
    session = new RecordSession(shadow)
    await session.start()
    const options = mockRecord.mock.calls[0][0] as MockRecordOptions
    ...
    options.emit({
      type: 4,
      data: {
        href: 'https://app.example.com/reset?token=secret#confirm',
        text: 'Card 4242 4242 4242 4242',
      },
    })

    expect(session.stop().events[1]).toEqual({
      type: 4,
      data: {
        href: 'https://app.example.com/reset',
        text: 'Card [Filtered]',
      },
    })
  })

  it('drops an event when filtering it fails', async () => {
    session = new RecordSession(shadow)
    await session.start()
    const options = mockRecord.mock.calls[0][0] as MockRecordOptions
    const invalidEvent = { type: 4 } as Record<string, unknown>
    Object.defineProperty(invalidEvent, 'data', {
      enumerable: true,
      get: () => { throw new Error('host getter failed') },
    })

    options.emit(invalidEvent)

    expect(session.stop().events).toEqual([{ type: 2, data: 'snapshot' }])
  })
  ...
  it('getDuration() returns elapsed time in seconds', async () => {
    ...
    expect(session.getDuration()).toBe(1)
  ...
  it('reports a minimum one-second duration after recording starts', async () => {
    session = new RecordSession(shadow)
    await session.start()

    expect(session.stop().duration).toBe(1)
  })
