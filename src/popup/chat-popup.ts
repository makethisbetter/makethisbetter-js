...
export class ChatPopup {
  ...
  private async sendMessage(text: string): Promise<void> {
    ...
    if (!bufferedDeltas.length && aiMessage) {
   ...
   * when it lands, and a failed finalize takes this card down in favour of its
   * own error card.
   */
  ...
  private attachHandoff(): void {
    ...
    } else if (!emailCapture && !this.autoCloseTimer && !view.querySelector('.mtb-chat-email-card')) {
