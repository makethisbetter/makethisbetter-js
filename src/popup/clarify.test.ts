...
describe('ClarifyCard', () => {
  ...
  it('sends user message via apiClient', async () => {
    ...
    await vi.waitFor(() => {
      expect(shadow.root.querySelector('.mtb-clarify-summary')).toBeTruthy()
    })
  ...
  it('keeps the answer editable when saving it fails', async () => {
    ...
    const { shadow, card, messages } = setupCard({ startClarification })
    await vi.waitFor(() => {
      expect(startClarification).toHaveBeenCalledOnce()
    })
    await vi.advanceTimersByTimeAsync(0)
    ...
    expect(shadow.root.querySelector('.mtb-clarify-summary')).toBeNull()
  ...
  it('shows a summary card and calls onFinalize on Send feedback when done=true from start', async () => {
    const startClarification = vi.fn().mockResolvedValue({
      status: 'complete',
  ...
  it('replaces the input with a summary send-feedback button after done', async () => {
    const startClarification = vi.fn().mockResolvedValue({
      status: 'complete',
    ...
    expect(shadow.root.querySelector('.mtb-clarify-input')).toBeNull()
    expect(shadow.root.querySelector('.mtb-clarify-send')).toBeNull()

    card.destroy()
    shadow.destroy()
  })

  it('renders the original comment, clarified Q/A and captured context in the summary', async () => {
    const startClarification = vi.fn().mockResolvedValue({
      status: 'complete',
      messages: [
        { role: 'assistant', content: 'What were you exporting?' },
        { role: 'user', content: 'The Q2 report' },
      ],
      done: true,
    } satisfies ClarifyResponse)
    ...
    const card = new ClarifyCard(shadow, {
      submissionSessionId: 'submission_123',
      submissionToken: 'submission-secret',
      apiClient: mockApiClient({ startClarification }),
      messages: getMessages('en'),
      onFinalize,
      summary: {
        originalDescription: 'Export is broken',
        element: 'Export PDF',
        page: '/reports/q2',
    ...
    const summary = shadow.root.querySelector('.mtb-clarify-summary')!
    expect(summary.querySelector('.mtb-summary-quote')!.textContent).toBe('Export is broken')
    expect(summary.querySelector('.mtb-summary-qa')!.textContent).toContain('What were you exporting?')
    expect(summary.querySelector('.mtb-summary-qa')!.textContent).toContain('The Q2 report')
    expect(summary.textContent).not.toContain('Export PDF')
    expect(summary.textContent).not.toContain('/reports/q2')

    card.destroy()
    shadow.destroy()
  })

  it('shows summary without finalizing on startClarification error', async () => {
  ...
  it('shows the summary card when polling exceeds the timeout with no reply', async () => {
    const getClarification = vi.fn().mockResolvedValue({
      status: 'pending',
      messages: [],
      done: false,
    ...
    // Poll past the 45s deadline; the card should give up and surface the
    // summary so the user can still confirm the send.
