...
import { computeCropRegion, cropCanvas } from './crop'
...
async function bakeAnnotations(dataUrl: string, annotations: Annotation[]): Promise<Blob | null> {
  ...
  const cropRect = computeCropRegion(annotations, window.innerWidth, window.innerHeight)
  if (cropRect) {
    const scaled = {
    ...
    return canvasToJpegBlob(cropCanvas(canvas, scaled))
  }
