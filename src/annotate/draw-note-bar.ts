...
interface DrawNoteBarOptions {
  ...
  onRedo: () => void
  onCancel: () => void
  onSubmit: (note: string, initiatingKeydown?: KeyboardEvent) => void
...
export class DrawNoteBar {
  ...
  private onSubmit: (note: string, initiatingKeydown?: KeyboardEvent) => void
  ...
  private submit(initiatingKeydown?: KeyboardEvent): void {
    this.onSubmit(this.input.value.trim(), initiatingKeydown)
  }
