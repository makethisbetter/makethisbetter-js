...
describe('AnnotationSession', () => {
  ...
  function drag(overlayEl: Element, points: number): void {
    overlayEl.dispatchEvent(new MouseEvent('mousedown', { clientX: 0, clientY: 0, bubbles: true }))
    for (let i = 1; i < points; i++) {
      overlayEl.dispatchEvent(new MouseEvent('mousemove', { clientX: i * 10, clientY: i * 10, bubbles: true }))
    }
    overlayEl.dispatchEvent(new MouseEvent('mouseup', { clientX: (points - 1) * 10, clientY: (points - 1) * 10, bubbles: true }))
  }
  ...
  describe('pin annotation via short click', () => {
    it('creates a pin annotation on mousedown + mouseup with fewer than 5 points', () => {
      ...
      overlayEl.dispatchEvent(new MouseEvent('mousedown', { clientX: 100, clientY: 200, bubbles: true }))
      overlayEl.dispatchEvent(new MouseEvent('mouseup', { clientX: 100, clientY: 200, bubbles: true }))
    ...
    it('adds a pin marker to the shadow root', () => {
      ...
      overlayEl.dispatchEvent(new MouseEvent('mousedown', { clientX: 50, clientY: 50, bubbles: true }))
      overlayEl.dispatchEvent(new MouseEvent('mouseup', { clientX: 50, clientY: 50, bubbles: true }))
    ...
    it('increments pin count on successive clicks', () => {
      ...
      for (let i = 0; i < 3; i++) {
        overlayEl.dispatchEvent(new MouseEvent('mousedown', { clientX: 10 * i, clientY: 10 * i, bubbles: true }))
        overlayEl.dispatchEvent(new MouseEvent('mouseup', { clientX: 10 * i, clientY: 10 * i, bubbles: true }))
      }
  ...
  describe('draw annotation via drag', () => {
    ...
    it('ignores stray short clicks once strokes exist (no pin)', () => {
      ...
      overlayEl.dispatchEvent(new MouseEvent('mousedown', { clientX: 200, clientY: 200, bubbles: true }))
      overlayEl.dispatchEvent(new MouseEvent('mouseup', { clientX: 200, clientY: 200, bubbles: true }))
  ...
  describe('hover highlight', () => {
    ...
    it('shows the highlight box over the element under the cursor', () => {
      ...
      overlayEl.dispatchEvent(new MouseEvent('mousemove', { clientX: 10, clientY: 10, bubbles: true }))
      flushFrame()
    ...
    it('suppresses hover highlight while a draw session is active, resumes after clearDraw', () => {
      ...
      session.undoDraw()
      overlayEl.dispatchEvent(new MouseEvent('mousemove', { clientX: 12, clientY: 12, bubbles: true }))
      flushFrame()
      expect(highlightEl.style.display).not.toBe('block')

      session.clearDraw()
      overlayEl.dispatchEvent(new MouseEvent('mousemove', { clientX: 14, clientY: 14, bubbles: true }))
      flushFrame()
      expect(highlightEl.style.display).toBe('block')
      session.destroy()
    })
