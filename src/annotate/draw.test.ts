...
describe('DrawLayer', () => {
  ...
  describe('startDraw / continueDraw', () => {
    it('creates a path and extends it as the pointer moves', () => {
      setup()
    ...
    it('continueDraw does nothing when not drawing', () => {
      ...
      expect(draw.getCurrentSpan()).toBe(0)
    })
  })

  // Travel, not point count, is what tells a tap from a stroke: a mouse click
  // reports no movement while a finger reports a burst of sub-pixel jitter.
  describe('getCurrentSpan', () => {
    ...
    it('is zero for a press that never moves', () => {
      setup()
      draw.startDraw(10, 20)
      expect(draw.getCurrentSpan()).toBe(0)
    })

    it('measures the furthest point from where the pointer went down', () => {
      setup()
      draw.startDraw(0, 0)
      draw.continueDraw(3, 4)
      expect(draw.getCurrentSpan()).toBe(5)
    })

    // A stroke that loops back must not read as a tap just because it ended
    // where it started.
    it('keeps the furthest distance even after returning to the start', () => {
      setup()
      draw.startDraw(0, 0)
      draw.continueDraw(30, 40)
      draw.continueDraw(0, 0)
      expect(draw.getCurrentSpan()).toBe(50)
    })

    it('resets once the stroke is cancelled', () => {
      setup()
      draw.startDraw(0, 0)
      draw.continueDraw(30, 40)
