...
export class WidgetController implements ModeHost {
  ...
  constructor(config: MakeThisBetterConfig) {
    ...
    this.lifecycle = new SubmissionLifecycle(
      ...
      },
      {
        ...
        onReleaseForFailureCard: () => {
          // The chat card is showing its receipt by now; the failure card is
          // the correction, so the receipt cannot stay next to it.
  ...
  mountDrawBar(): void {
    ...
    this.drawBar = new DrawNoteBar(this.shadow, {
      ...
      onActivity: () => this.lifecycle.warmScreenshotDeps(),
    })
  ...
  mountNotePopup({ x, y, targetName, targetRect }: NotePopupOptions): void {
    ...
    this.chatPopup = new ChatPopup(this.shadow, {
      ...
      // and the scroll lock have nothing left to hold, and a failure from here
      // on has to reach the reporter through its own card rather than a
      // conversation that has already said thank you.
      onFinalize: () => {
