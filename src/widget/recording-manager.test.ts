...
// The module-level mocks are never cleared between tests, so results[0] is the
// first start() in the whole file — assert on the instance this test created.
function lastInstance(constructorMock: unknown): { destroy: ReturnType<typeof vi.fn> } {
  const results = (constructorMock as ReturnType<typeof vi.fn>).mock.results
  return results[results.length - 1].value as { destroy: ReturnType<typeof vi.fn> }
}
...
describe('RecordingManager', () => {
  ...
  it('destroy cleans up session and bar', async () => {
    ...
    const { RecordSession } = await import('../record/session')
    const sessionInstance = lastInstance(RecordSession)
    const { RecordControlBar } = await import('../record/control-bar')
    const barInstance = lastInstance(RecordControlBar)
