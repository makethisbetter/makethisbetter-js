...
// One happy-path server: session created, clarification immediately done (the
// card shows its confirm button), finalize succeeds, abandon accepted.
...
/** Runs the full submit -> clarify confirm -> success terminal flow. */
...
describe('the mode machine', () => {
  ...
  describe('Escape per mode', () => {
    ...
    // CHARACTERIZED SURPRISE: clarifying is the one active mode Escape does not
    // touch — only the card's X cancels. The card holds an in-flight
    // submission, so this asymmetry is probably deliberate; pinned so the
    // refactor decides rather than inherits.
    it('leaves the clarify conversation alone', async () => {
  ...
  describe('terminal flows', () => {
    it('submit -> clarify -> confirm -> success lands in idle with the toolbar gone', async () => {
      const shadow = initWidget()
    ...
    it('skipping the clarification finalizes in the background with an inert launcher', async () => {
      ...
      typeAndSubmit(shadow)
      await vi.waitFor(() => {
        expect(shadow.querySelector('.mtb-clarify-skip')).not.toBeNull()
      }, { timeout: 5000 })
      ...
      // state: the card and toolbar are down, and the launcher must not start
      // a second feedback on top of the one still finalizing.
      expect(shadow.querySelector('.mtb-clarify')).toBeNull()
      expect(shadow.querySelector('.mtb-toolbar')).toBeNull()
      shadow.querySelector<HTMLButtonElement>('.mtb-tab')!.click()
      expect(shadow.querySelector('.mtb-overlay')).toBeNull()
      ...
      // The background finalize then completes into the success card.
      await vi.waitFor(() => {
        expect(shadow.querySelector('.mtb-success')).not.toBeNull()
      }, { timeout: 5000 })
    })
  ...
  describe('re-entrancy', () => {
    it('a second session after a full submit-success cycle is fully functional', async () => {
      ...
      placePin(shadow)
      expect(shadow.querySelector('.mtb-popup')).not.toBeNull()
      expect(shadow.querySelector('.mtb-textarea')).not.toBeNull()
    })
  })
})
