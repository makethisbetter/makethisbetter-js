...
// Phase-2 pet mode placeholder: not wired into the widget build and not part
// of the public surface, so it stays unexported until the feature ships.
class PetEntry {
  ...
  private buildSVG(): string {
    return `<svg class="mtb-pet-svg" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="24" cy="28" r="16" fill="var(--mtb-brand-primary)"/>
      <circle cx="24" cy="28" r="14" fill="var(--mtb-brand-emphasis)"/>
      ...
      <circle cx="15" cy="30" r="2.5" fill="var(--mtb-brand-primary)" opacity="0.4"/>
      <circle cx="33" cy="30" r="2.5" fill="var(--mtb-brand-primary)" opacity="0.4"/>
      <path d="M14 17 Q16 12 20 15" stroke="var(--mtb-brand-emphasis)" stroke-width="2" stroke-linecap="round" fill="none"/>
      <path d="M34 17 Q32 12 28 15" stroke="var(--mtb-brand-emphasis)" stroke-width="2" stroke-linecap="round" fill="none"/>
    </svg>`
  }
