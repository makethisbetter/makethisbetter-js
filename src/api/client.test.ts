import { describe, it, expect, vi, afterEach } from 'vitest'
import { ApiClient, FINALIZE_TIMEOUT_MS } from './client'
import type { FeedbackPayload } from '../types'

const mockPayload: FeedbackPayload = {
  description: 'Test feedback',
  page_url: 'https://example.com',
  user_agent: 'Mozilla/5.0',
  browser: 'Chrome 130',
  os: 'macOS',
  screen_width: 1440,
  screen_height: 900,
  console_errors: [],
  annotations: [],
}

describe('ApiClient', () => {
  it('requests a Session-scoped proxy URL only for failed cross-origin HTTP images', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      url: 'https://image-proxy.example.com/signed',
    }), { status: 201 }))
    const client = new ApiClient({ projectKey: 'acme key', apiUrl: 'https://api.example.com/api/v1/' })

    await expect(client.resolveScreenshotResourceUrl(
      'submission_1',
      'secret',
      'https://cdn.example.com/photo.png',
    )).resolves.toBe('https://image-proxy.example.com/signed')
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://api.example.com/api/v1/widget/feedback_submission_sessions/submission_1/image_proxy_url',
      expect.objectContaining({
        method: 'POST',
        headers: expect.objectContaining({
          'Content-Type': 'application/json',
          'X-Project-Key': 'acme key',
          'X-Submission-Token': 'secret',
        }),
        body: JSON.stringify({ image_proxy_url: { source: 'https://cdn.example.com/photo.png' } }),
      }),
    )

    await expect(client.resolveScreenshotResourceUrl('submission_1', 'secret', `${window.location.origin}/photo.png`)).resolves.toBeUndefined()
    await expect(client.resolveScreenshotResourceUrl('submission_1', 'secret', 'data:image/png;base64,abc')).resolves.toBeUndefined()
    await expect(client.resolveScreenshotResourceUrl('submission_1', 'secret', 'blob:https://example.com/id')).resolves.toBeUndefined()
    await expect(client.resolveScreenshotResourceUrl('submission_1', 'secret', 'https://cdn.example.com/font.woff2', 'application/font-woff')).resolves.toBeUndefined()
    expect(fetchSpy).toHaveBeenCalledTimes(1)
  })

  it('uses the transparent placeholder when the optional proxy endpoint is unavailable', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, { status: 404 }))
    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://self-hosted.example.com/api/v1' })

    await expect(client.resolveScreenshotResourceUrl(
      'submission_1',
      'secret',
      'https://cdn.example.com/photo.png',
    )).resolves.toBeUndefined()
  })

  afterEach(() => {
    vi.restoreAllMocks()
  })

  it('sends POST to correct URL with project key header', async () => {
    const jsonBody = JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(jsonBody, { status: 201 }),
    )

    const client = new ApiClient({ projectKey: 'acme123', apiUrl: 'https://api.example.com' })
    await client.createSubmissionSession(mockPayload, null)

    expect(fetchSpy).toHaveBeenCalledWith(
      'https://api.example.com/widget/feedback_submission_sessions',
      expect.objectContaining({
        method: 'POST',
        headers: expect.objectContaining({ 'X-Project-Key': 'acme123' }),
      }),
    )
  })

  it('declares the fixed sensitive-data capture policy for session media', async () => {
    const jsonBody = JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(jsonBody, { status: 201 }),
    )

    const client = new ApiClient({ projectKey: 'acme123', apiUrl: 'https://api.example.com' })
    await client.createSubmissionSession(mockPayload, null)

    expect(fetchSpy).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        headers: expect.objectContaining({
          'X-MTB-Capture-Policy': 'sensitive-data-v1',
        }),
      }),
    )
  })

  it('includes description in FormData', async () => {
    let capturedBody: FormData | null = null
    const jsonBody = JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } })
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (_url: RequestInfo | URL, init?: RequestInit) => {
      capturedBody = init?.body instanceof FormData ? init.body : null
      return new Response(jsonBody, { status: 201 })
    })

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    await client.createSubmissionSession(mockPayload, null)

    expect(capturedBody).not.toBeNull()
    const formData = capturedBody as unknown as FormData
    expect(formData.get('feedback[description]')).toBe('Test feedback')
  })

  it('uploads screenshot when present', async () => {
    let capturedBody: FormData | null = null
    const jsonBody = JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } })
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (_url: RequestInfo | URL, init?: RequestInit) => {
      capturedBody = init?.body instanceof FormData ? init.body : null
      return new Response(jsonBody, { status: 201 })
    })

    const screenshot = new Blob(['png'], { type: 'image/png' })
    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    await client.createSubmissionSession(mockPayload, screenshot)

    const formData = capturedBody as unknown as FormData
    expect(formData.get('feedback[screenshot]')).toBeInstanceOf(File)
  })

  it('includes X-User-Token header when userToken is set', async () => {
    const jsonBody = JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(jsonBody, { status: 201 }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com', userToken: 'jwt_static_token' })
    await client.createSubmissionSession(mockPayload, null)

    expect(fetchSpy).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        headers: expect.objectContaining({
          'X-Project-Key': 'key',
          'X-User-Token': 'jwt_static_token',
        }),
      }),
    )
  })

  it('calls userTokenFn to get fresh token before each request', async () => {
    const jsonBody = JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(jsonBody, { status: 201 }),
    )

    const tokenFn = vi.fn(async () => 'jwt_dynamic_token')
    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com', userTokenFn: tokenFn })
    await client.createSubmissionSession(mockPayload, null)

    expect(tokenFn).toHaveBeenCalledOnce()
    expect(fetchSpy).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        headers: expect.objectContaining({
          'X-User-Token': 'jwt_dynamic_token',
        }),
      }),
    )
  })

  it('prefers userTokenFn over static userToken', async () => {
    const jsonBody = JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(jsonBody, { status: 201 }),
    )

    const tokenFn = vi.fn(async () => 'from_fn')
    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com', userToken: 'from_static', userTokenFn: tokenFn })
    await client.createSubmissionSession(mockPayload, null)

    expect(fetchSpy).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        headers: expect.objectContaining({
          'X-User-Token': 'from_fn',
        }),
      }),
    )
  })

  it('includes X-User-Token in answerClarification', async () => {
    const jsonBody = JSON.stringify({ clarification: { status: 'pending', messages: [], done: false } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(jsonBody, { status: 200 }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com', userToken: 'jwt_token' })
    await client.answerClarification('submission_1', 'secret', 'hello')

    expect(fetchSpy).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        headers: expect.objectContaining({
          'X-Project-Key': 'key',
          'X-Submission-Token': 'secret',
          'X-User-Token': 'jwt_token',
          'Content-Type': 'application/json',
        }),
      }),
    )
  })

  it('filters sensitive content before sending a clarification answer', async () => {
    const jsonBody = JSON.stringify({ clarification: { status: 'done', messages: [], done: true } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(jsonBody, { status: 200 }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    await client.answerClarification('submission_1', 'secret', 'Card 4242 4242 4242 4242')

    expect(fetchSpy).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        body: JSON.stringify({ message: 'Card [Filtered]' }),
      }),
    )
  })

  it('throws after retries exhausted', async () => {
    vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('Network error'))

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com', retryDelayMs: 0 })
    await expect(client.createSubmissionSession(mockPayload, null)).rejects.toThrow()
  })

  it('reuses the idempotency key and submission token across retries', async () => {
    const calls: RequestInit[] = []
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (_url, init) => {
      calls.push(init!)
      if (calls.length === 1) return new Response('oops', { status: 500 })
      return new Response(JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } }), { status: 201 })
    })

    const client = new ApiClient({ projectKey: 'acme123', apiUrl: 'https://api.example.com', retryDelayMs: 0 })
    await client.createSubmissionSession(mockPayload, null)

    expect(fetchSpy).toHaveBeenCalledTimes(2)
    const key1 = (calls[0].headers as Record<string, string>)['X-Idempotency-Key']
    const key2 = (calls[1].headers as Record<string, string>)['X-Idempotency-Key']
    const token1 = (calls[0].headers as Record<string, string>)['X-Submission-Token']
    const token2 = (calls[1].headers as Record<string, string>)['X-Submission-Token']
    expect(key1).toBeTruthy()
    expect(key2).toBe(key1)
    expect(token1.length).toBeGreaterThanOrEqual(32)
    expect(token2).toBe(token1)
  })

  it('streams clarify deltas and resolves the done result', async () => {
    const sse = [
      'event: delta\ndata: {"text":"Hi "}\n\n',
      'event: delta\ndata: {"text":"there"}\n\n',
      'event: done\ndata: {"done":true,"messages":[{"role":"assistant","content":"Hi there"}]}\n\n',
    ].join('')
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(sse, { status: 200, headers: { 'Content-Type': 'text/event-stream' } }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    const deltas: string[] = []
    const result = await client.streamClarification('submission_1', 'secret', (t) => deltas.push(t))

    expect(deltas).toEqual(['Hi ', 'there'])
    expect(result.done).toBe(true)
    expect(result.messages).toEqual([{ role: 'assistant', content: 'Hi there' }])
  })

  it('reports a failed clarify stream instead of treating it as complete', async () => {
    const sse = 'event: done\ndata: {"error":true,"done":true}\n\n'
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(sse, { status: 200, headers: { 'Content-Type': 'text/event-stream' } }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    const result = await client.streamClarification('submission_1', 'secret', () => {})

    expect(result.failed).toBe(true)
    expect(result.messages).toEqual([])
  })

  it('gives up on a clarify stream that is never terminated', async () => {
    // A stream the server accepts but never finishes: without a deadline the
    // reporter would sit on the thinking state with no retry and no way to skip.
    vi.spyOn(globalThis, 'fetch').mockImplementation((_url, init) => {
      const signal = (init as RequestInit | undefined)?.signal
      return new Promise((_resolve, reject) => {
        const abort = () => reject(new DOMException('Aborted', 'AbortError'))
        if (signal?.aborted) return abort()
        signal?.addEventListener('abort', abort)
      })
    })

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    const controller = new AbortController()
    const streaming = client.streamClarification('submission_1', 'secret', () => {}, controller.signal)
    setTimeout(() => controller.abort(), 10)

    await expect(streaming).rejects.toThrow(/abort/i)
  })

  it('rejects a clarify stream that ends before the done event', async () => {
    const sse = 'event: delta\ndata: {"text":"Partial reply"}\n\n'
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(sse, { status: 200, headers: { 'Content-Type': 'text/event-stream' } }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })

    await expect(client.streamClarification('submission_1', 'secret', () => {}))
      .rejects.toThrow('clarify stream ended before done')
  })

  it('throws from streamClarification when the server does not stream', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ done: false }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    await expect(client.streamClarification('submission_1', 'secret', () => {})).rejects.toThrow()
  })

  it('uses a fresh idempotency key for each separate submission', async () => {
    const keys: string[] = []
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (_url, init) => {
      keys.push((init!.headers as Record<string, string>)['X-Idempotency-Key'])
      return new Response(JSON.stringify({ submission_session: { id: 'submission_1', token: 'secret' } }), { status: 201 })
    })

    const client = new ApiClient({ projectKey: 'acme123', apiUrl: 'https://api.example.com' })
    await client.createSubmissionSession(mockPayload, null)
    await client.createSubmissionSession(mockPayload, null)

    expect(keys[0]).toBeTruthy()
    expect(keys[0]).not.toBe(keys[1])
  })

  it('finalizes a submission session only with its token', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ feedback: { id: 'FB-1', status: 'received', project_id: 'acme' } }), { status: 201 }),
    )

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    const feedback = await client.finalizeSubmissionSession('submission_1', 'secret')

    expect(feedback.id).toBe('FB-1')
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://api.example.com/widget/feedback_submission_sessions/submission_1/feedback',
      expect.objectContaining({
        method: 'POST',
        headers: expect.objectContaining({ 'X-Submission-Token': 'secret' }),
      }),
    )
  })

  it('gives up on a finalize that never settles', async () => {
    vi.useFakeTimers()
    // A reporter who skipped the clarification has no card left to report a
    // stall, so the request itself has to end.
    let attempts = 0
    const aborted: boolean[] = []
    vi.spyOn(globalThis, 'fetch').mockImplementation((_url, init?: RequestInit) => {
      attempts += 1
      return new Promise((_resolve, reject) => {
        init?.signal?.addEventListener('abort', () => {
          aborted.push(true)
          reject(new DOMException('Aborted', 'AbortError'))
        })
      })
    })

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    const finalize = client.finalizeSubmissionSession('submission_1', 'secret')
    const settled = finalize.then(() => 'resolved').catch(() => 'rejected')

    await vi.advanceTimersByTimeAsync(FINALIZE_TIMEOUT_MS - 1)
    expect(aborted).toHaveLength(0)

    await vi.advanceTimersByTimeAsync(2)
    expect(await settled).toBe('rejected')
    // The deadline is spent: a retry would only wait out the backoff for a
    // second guaranteed abort.
    expect(attempts).toBe(1)

    vi.useRealTimers()
  })

  it('abandons a submission session on Exit', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(null, { status: 204 }))

    const client = new ApiClient({ projectKey: 'key', apiUrl: 'https://api.example.com' })
    await client.abandonSubmissionSession('submission_1', 'secret')

    expect(fetchSpy).toHaveBeenCalledWith(
      'https://api.example.com/widget/feedback_submission_sessions/submission_1',
      expect.objectContaining({
        method: 'DELETE',
        headers: expect.objectContaining({ 'X-Submission-Token': 'secret' }),
      }),
    )
  })
})
